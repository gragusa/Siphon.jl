...
spec_diag = dns_model(maturities;
...
println("Profile LL range: [", round(minimum(result_em.loglik_profile), digits=1),
...
spec_diag_T_full = dns_model(maturities;
    T_structure=:full,
...
result_diag = profile_em_ssm(spec_diag_T_full, yields;
    λ_grid=0.02:0.01:0.12,
    verbose=false,
    maxiter=200
)

println("Full Q model:     λ = ", round(result_em.λ_optimal, digits=4),
        ", loglik = ", round(result_em.loglik, digits=2))
println("Diagonal Q model: λ = ", round(result_diag.λ_optimal, digits=4),
        ", loglik = ", round(result_diag.loglik, digits=2))
println("Log-likelihood improvement: ", round(result_em.loglik - result_diag.loglik, digits=2))
...
if isfile(data_path)
    ...
    spec_real = dns_model(selected_maturities;
        T_structure=:full,
        H_structure=:diagonal,
        Q_structure=:full,
        λ_init=0.06,
    ...
    result_real = profile_em_ssm(spec_real, selected_yields;
    ...
    p_real = KFParms(Z_real, result_real.em_result.H, T_real,
                     Matrix{Float64}(I, 3, 3), Q_real)
    a1_real = zeros(3)
    P1_real = 1e4 * Matrix{Float64}(I, 3, 3)

    filt_real = kalman_filter(p_real, selected_yields, a1_real, P1_real)
    smooth_real = kalman_smoother(Z_real, T_real, filt_real.at, filt_real.Pt,
                                   filt_real.vt, filt_real.Ft)
