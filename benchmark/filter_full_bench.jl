...
function benchmark_filter_scalar(; lengths = [100, 1000, 10000])
    ...
    for n in lengths
        ...
        print("Pure:     ")
        t_pure = @benchmark filter_scalar_pure(
            $y,
            $(parms.Z),
            $(parms.H),
            $(parms.T),
        ...
        print("In-place: ")
        t_inplace = @benchmark filter_scalar_inplace(
            $y,
            $(parms.Z),
            $(parms.H),
            $(parms.T),
...
function benchmark_filter_small(; lengths = [100, 1000, 10000])
    ...
    for n in lengths
        ...
        print("In-place: ")
        t_inplace = @benchmark filter_inplace(
            $y,
            $(parms.Z),
            $(parms.H),
            $(parms.T),
        ...
        # Memory comparison
        alloc_pure = @allocated filter_pure(
            y,
            parms.Z,
            parms.H,
            parms.T,
...
function benchmark_comparison_table(; lengths = [100, 1000, 10000])
    ...
    for (model, m, n, t_pure, t_inplace) in results
        ...
        println(
            "| $(rpad(model, 6)) | $(lpad(m, 6)) | $(lpad(n, 5)) | $(lpad(round(t_pure/1e6, digits=3), 9)) | $(lpad(round(t_inplace/1e6, digits=3), 13)) | $(lpad(round(speedup, digits=2), 7)) |",
        )
    end
